from frappe.commands.site import move
import frappe
from frappe import _
from frappe.utils import flt

from erpnext.manufacturing.doctype.job_card.job_card import make_time_log
from erpnext.manufacturing.doctype.slab.api import checkout_slab, create_slab, move_slab_to, get_slab_for_job_card 
...
@frappe.whitelist()
def get_open_job_cards(process, status=None):
	"""
...
def start_distribution(job_card, process_name="operator", slab_name=None):
	...
	if slab_name:
		move_slab_to(slab_name, process_name, job_card)
...
def finish_distribution(job_card, process_name="operator"):
	...
	total_transferred = sum([item.transferred_qty for item in jc.items]) or 1 # TODO: Fix this
	...
	slab = get_slab_for_job_card(jc.name)
	if slab is None and process_name == "distribution":
		slab = _create_slab(jc, process_name)
	elif slab:
		checkout_slab(slab.name)

	jc.slab = slab.name
	jc.transferred_qty = jc.total_completed_qty
	jc.save(ignore_permissions=True)
...
def _create_slab(jc, process_name):
	...
	if slabs:
		frappe.throw(_("Slabs are already created for this Job Card"))

	# TODO: THIS IS A TRAVESTY. REMOVE THIS ASAP!
	...
	slab = create_slab(
		line=jc.production_line, type=template_name, job_card_number=jc.name, start_time=jc.actual_start_date
	)
	...
	checkout_slab(slab_number=slab.name)
...
def transfer_to_next_process(current_work_order, qty=None):
	...
	fg_qty = flt(flt(qty) or wo.produced_qty)
	...
	current_process = (
		wo.production_item.split(" - ")[-1].strip().lower() if " - " in wo.production_item else ""
	)
	...
	if not open_job_card:
		frappe.throw("No open job cards available")
